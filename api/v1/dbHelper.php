...
class dbHelper {
    ...
    function exists($table, $where){
        try{
            ...
            $stmt = $this->db->prepare("select count(badgeid) from ".$table." where 1=1 ". $w);
            $stmt->execute($a);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $response["data"] = $rows;
        }catch(PDOException $e){
    ...
    function selectJoin($table1, $table2, $columns, $on){
        try{
            $a = array();
            $stmt = $this->db->prepare("select ".$columns." from ".$table1." left join ".$table2." on ". $on);
            $stmt->execute($a);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            if(count($rows)<=0){
                $response["status"] = "warning";
